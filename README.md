...
Клавиши: `enter` — детали, `o` — браузер, `r` — обновить вкладку, `R` — синк всего,
`c` — свернуть панель изменений, `q` — выход.

Без TUI: `jwu sync` — разовый полный синк.

## 5. Скиллы для Claude

Промты скиллов лежат в репозитории: **`src/jwu/skills/<имя>/SKILL.md`** — их можно взять прямо из git.

**Claude Code** — установить одной командой (копирует в `~/.claude/skills`, обновляя существующие):

```bash
jwu install-claude-skills          # каталог можно сменить флагом --dest
```

**Другой агент / не Claude** — просто скорми содержимое нужного `SKILL.md` из `src/jwu/skills/`
...
| Скилл | Когда / как |
|---|---|
| `/jwu-start-job` | «начни работу по ABC-123», «проанализируй задачу» — анализ + план + создание работы |
...
| `jwu install-claude-skills` | развернуть скиллы в `~/.claude/skills` |
